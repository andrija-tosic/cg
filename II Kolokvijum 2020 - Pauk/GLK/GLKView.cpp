...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_UP) m_glRenderer.xRot += 5;
	if (nChar == VK_DOWN) m_glRenderer.xRot -= 5;

	if (nChar == VK_ADD) m_glRenderer.zoom += 5;
	if (nChar == VK_SUBTRACT) m_glRenderer.zoom -= 5;

	if (nChar == 'W') m_glRenderer.spiderX += 5;
	if (nChar == 'S') m_glRenderer.spiderX -= 5;

	if (nChar == 'A') m_glRenderer.spiderYRot += 5;
	if (nChar == 'D') m_glRenderer.spiderYRot -= 5;

	if (nChar == VK_NUMPAD8) m_glRenderer.qRot += 5;
	if (nChar == VK_NUMPAD2) m_glRenderer.qRot -= 5;
