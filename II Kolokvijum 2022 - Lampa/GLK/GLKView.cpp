...
{
	if (nChar == VK_LEFT) m_glRenderer.yRot += 5;
	if (nChar == VK_RIGHT) m_glRenderer.yRot -= 5;

	if (nChar == VK_UP) {
		m_glRenderer.xRot += 0.1*toDeg;
		m_glRenderer.xRot = min(m_glRenderer.xRot, 1.5*toDeg);

	}

	if (nChar == VK_DOWN) {
		m_glRenderer.xRot -= 0.1*toDeg;
		m_glRenderer.xRot = max(m_glRenderer.xRot, 0.1*toDeg);
	}

	if (nChar == VK_ADD) m_glRenderer.zoom += 5;
	if (nChar == VK_SUBTRACT) m_glRenderer.zoom -= 5;

	if (nChar == '1') m_glRenderer.arm1Rot -= 5;
	if (nChar == '2') m_glRenderer.arm1Rot += 5;

	if (nChar == '3') m_glRenderer.arm2Rot -= 5;
	if (nChar == '4') m_glRenderer.arm2Rot += 5;

	if (nChar == '5') m_glRenderer.headRot -= 5;
	if (nChar == '6') m_glRenderer.headRot += 5;
